...
import abbvieSectionsTransformer from './transformers/sections.js';
...
const PAGE_TEMPLATE = {
  ...
  sections: [
    ...
    {
      id: 'section-3',
      name: 'Dashboard Stats + Core Focus',
...
/**
 * Consolidate multiple Section Metadata tables within the same section
 * (between HR boundaries) into a single table. EDS decorateSections only
 * processes the first Section Metadata per section.
 */
function consolidateSectionMetadata(root) {
  const children = Array.from(root.children);
  let sectionMetas = [];

  function merge() {
    if (sectionMetas.length <= 1) {
      sectionMetas = [];
      return;
    }
    // Keep the last SM (which is at the end of the section, where EDS expects it)
    const primary = sectionMetas[sectionMetas.length - 1];
    for (let i = 0; i < sectionMetas.length - 1; i++) {
      const secondary = sectionMetas[i];
      // Move data rows (skip header row) from secondary into primary
      const secondaryRows = Array.from(secondary.rows);
      for (let r = 1; r < secondaryRows.length; r++) {
        primary.appendChild(secondaryRows[r]);
      }
      secondary.remove();
    }
    sectionMetas = [];
  ...
  children.forEach((child) => {
    if (child.tagName === 'HR') {
      ...
      const name = child.rows && child.rows[0] && child.rows[0].cells
        && child.rows[0].cells[0] && child.rows[0].cells[0].textContent.trim().toLowerCase();
      if (name === 'section metadata') {
        sectionMetas.push(child);
      }
    }
  });
  merge(); // Handle last section
}
...
export default {
  ...
  transform: (payload) => {
    ...
    // 5b. Consolidate multiple Section Metadata blocks per section into one.
    // transformBackgroundImages may create a background SM separate from the
    // style SM created by the sections transformer. EDS decorateSections only
    // processes the first SM per section, so they must be merged.
    consolidateSectionMetadata(main);

    // 5c. Page metadata
    const meta = extractPageMetadata(document);
    const metaBlock = createMetadataBlock(document, meta, 'science-page');
