...
## Rule 7: AEM Component → EDS Block Mapping (Verified)

**All migration work MUST use the verified AEM→EDS block mappings. Never use generic EDS block names (hero, title, text, image) — always use the AbbVie custom blocks.**

### Canonical Source: `tools/importer/block-mapping.md`

**Before any migration task, READ `tools/importer/block-mapping.md`** — it contains:
- **Section 1:** Block Name Registry (13 blocks — which AEM class maps to which EDS block)
- **Section 2:** Variant Registry (exact variant classes for each block, per AEM source indicator)
- **Section 3:** Grid Layout Mapping (AEM grid-row → EDS Grid Container/Grid Section)
- **Section 4:** Section Container Mapping (AEM container classes → EDS section styles)
- **Section 5:** Reference Pages (6 verified source↔EDS pairs)
- **Section 6:** Changelog (track new blocks/variants added over time)

### Quick Reference (key block names — full details in block-mapping.md)

| AEM Component | EDS Block | NEVER Use |
|---|---|---|
| `cmp-container-full-width` (hero) | `hero-container` | ~~hero~~ |
| `cmp-title-*` | `custom-title` | ~~title~~ |
| `cmp-text-*` | `text-container` | ~~text~~ |
| `cmp-image` | `custom-image` | ~~image~~ |
| `cmp-button` | `cta` | ~~button~~ |
| `brightcove-video` | `brightcove-video` | ~~embed~~ |
| `.cardpagestory` | `story-card` | ~~cards~~ |
| `grid-row__col-with-N` | Grid Section `grid-cols-N` | ~~columns~~ |

### Enforcement

1. Before creating any import parser: **read** `tools/importer/block-mapping.md`
2. When a new block or variant is discovered: **update** `block-mapping.md` and add a changelog entry
3. Never hardcode block names in parsers — reference the registry

## Rule 8: Content Delivery via JCR Content Packages

**All migrated content MUST be delivered to AEM Author as JCR content packages (.zip), NOT via md2jcr conversion.**

### Why

md2jcr has a known bug with container blocks (text-container, hero-container) containing multi-paragraph richtext child items. The `FieldGroupFieldResolver.resolve()` fails for plain paragraph nodes. This affects ALL pages — including manually authored ones.

### Migration Output Pipeline

```
Source page (abbvie.com)
  → Import parsers (extract content, map to EDS blocks)
  → .plain.html (EDS block table format, for preview)
  → JCR XML (generated from imported content using reference XML structure)
  → AEM Content Package (.zip with META-INF/vault + jcr_root)
  → Install via AEM Package Manager (crx/packmgr)
```
...
- `sling:resourceType`: `core/franklin/components/block/v1/block` for blocks, `block/v1/block/item` for child items, `section/v1/section` for sections
- `modelFields` array MUST be present on every block/item — format: `[fieldName@componentType,...]`
- `aueComponentId` MUST match the component definition ID
- Richtext content is HTML-encoded in the `text` attribute: `&lt;p&gt;content&lt;/p&gt;`
- Boolean values use `{Boolean}true` / `{Boolean}false` format
- Long/number values use `{Long}N` format
- Array values use `[value1,value2]` format for `classes` multiselect
- Comma-separated values for `classes_customDynamicClass`
- Always use `component-models.json` (built) as field source, NOT `_block.json` (raw) — they differ after merge

### Reference

See `tools/importer/block-mapping.md` Section 6 for complete JCR XML node structure and package format.
